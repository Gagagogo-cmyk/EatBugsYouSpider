// band_mask_init.js — per-stem companion to gnumbat-pitch.maxpat's frequency-band
// gating. Each of the 4 pfft~ instances loads its OWN copy of gnumbat-pitch.maxpat
// with a distinct "args <stemshort>" (see gnumbat-analyze.maxpat's 4 pfft~ boxes:
// "pfft~ gnumbat-pitch.maxpat 1024 4 args voc/mel/bss/drm"), which substitutes
...
// stem suffix. That's how "gnumbat_pitch_mask_#1"/"gnumbat_formant_mask_#1" resolve
// to 4 DISTINCT buffer~ names (gnumbat_pitch_mask_voc, _mel, _bss, _drm, and
// likewise for formant) instead of one shared/broadcast buffer the way
// gnumbat_pitchWindow and gnumbat_formant_lifter are (those are intentionally
...
var MASK_BINS = 512;   // must match gnumbat-pitch.maxpat's buffer~/fft~ sizing
...
function fillFullPass(bufferName) {
    ...
    if (!b) {
        post("band_mask_init: buffer '" + bufferName + "' not found — is it declared in gnumbat-pitch.maxpat?\n");
        return;
    }
...
function loadbang() {
    if (!stem) {
        post("band_mask_init: no stem suffix passed via jsarguments — check gnumbat-analyze.maxpat's \"args <stem>\" on this pfft~ instance\n");
        return;
    }
    fillFullPass("gnumbat_pitch_mask_" + stem);
    fillFullPass("gnumbat_formant_mask_" + stem);
